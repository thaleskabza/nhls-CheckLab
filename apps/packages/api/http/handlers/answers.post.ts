...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.checklistId !== id) {
    return new Response(JSON.stringify({ error: "IdMismatch" }), { status: 400 });
  }

  const repo = PostgresChecklistRepo();
  try {
    const result = await (await import("@core/usecases/submitSection"))
      .submitSectionUseCase(repo, {
    ...
    if (String(e.message) === "VersionConflict") {
      return new Response(JSON.stringify({ error: "VersionConflict" }), { status: 409 });
    }
    if (String(e.message) === "NotEditable") {
      return new Response(JSON.stringify({ error: "NotEditable" }), { status: 400 });
    }
    throw e;
  }
}
